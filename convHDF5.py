...
def add_instances(unique_words, labels, words, num_to_word, pictures):
   ...
   for item in range(unique_words):
      ...
      # we want to sample from however many different images we have
      # cur_pic will iterate through the list of indices
      cur_pic = 0
...
def main():
   ...
   if LOAD_FROM_PICKLE:
      h5f = h5py.File('loaded_data.h5', 'r')
      training_labels = h5f['training_labels'][:]
      training_words  = h5f['training_words'][:]
      # pictures        = h5f['pictures'][:]
      ...
      h5f = h5py.File('loaded_data.h5', 'w')
      h5f.create_dataset('training_labels', data=training_labels)
      h5f.create_dataset('training_words',  data=training_words)
      # h5f.create_dataset('pictures', data=pictures)
